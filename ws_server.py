...
# 音频录制线程
async def audio_recorder(websocket):
    ...
    try:
        async for message in websocket:
            # 收到音频数据
            audio_data = message  # 假设发送的音频数据是二进制格式
            ...
            if time.time() - last_active_time > NO_SPEECH_THRESHOLD:
                ...
                if segments_to_save and segments_to_save[-1][1] > last_vad_end_time:
                    ...
                    if output_list:
                        print("发送回答")
                        # await websocket.send(output_list[0])
                        # output_list = []
                else:
                    pass
                    # print("无新增语音段，跳过保存")
...
# 启动 WebSocket 服务端
async def main():
    server = await websockets.serve(audio_recorder, "0.0.0.0", 8765)    # 等待 60 秒未收到“pong”则关闭连接
    print("WebSocket 服务端已启动，等待连接...")
